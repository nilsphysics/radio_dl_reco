...
class simulation:
    ...
    def __init__(self, inputfilename,
        ...
        if isinstance(inputfilename, str):
            logger.info(f"reading input from {inputfilename}")
            ...
            logger.info("getting input on-the-fly")
        ...
        if len(self._fin['xx']) == 0:
            logger.info(f"input file {self._inputfilename} is empty")
            return
...
#                 logger.warning('internal sampling rate is {:.3g}GHz, final detector sampling rate is {:.3g}GHz'.format(self.get_sampling_rate(), self._sampling_rate_detector))
        ...
        if noise_temp is not None:
            ...
            if noise_temp == "detector":
                logger.info("Use noise temperature from detector description to determine noise Vrms in each channel.")
                self._noise_temp = None  # the noise temperature is defined in the detector description
            else:
                self._noise_temp = float(noise_temp)
                logger.info(f"Use a noise temperature of {noise_temp / units.kelvin:.1f} K for each channel to determine noise Vrms.")
            ...
            for station_id in self._integrated_channel_response:
                for channel_id in self._integrated_channel_response[station_id]:
                    ...
                    logger.info(f'Station.channel {station_id}.{channel_id:02d}: noise temperature = {noise_temp_channel} K, '
            ...
            logger.info(f"Use a fix noise Vrms of {self._Vrms / units.mV:.2f} mV in each channel.")
            ...
            for station_id in self._integrated_channel_response:
                ...
                for channel_id in self._integrated_channel_response[station_id]:
                    ...
                    logger.info(f'Station.channel {station_id}.{channel_id:02d}: '
                                  f'est. bandwidth = {integrated_channel_response / mean_integrated_response / units.MHz:.2f} MHz, '
        ...
        logger.info(f"All signals with less then {speed_cut:.1f} x Vrms_efield will be skipped.")
    ...
    def run(self):
        ...
        """
        if len(self._fin['xx']) == 0:
            logger.info(f"writing empty hdf5 output file")
...
#                     input_time += (time.time() - t1)
                    ...
                    channel_amplitude_theta = np.random.uniform(1, 10)
                    channel_amplitude_phi = np.random.uniform(1, 10)
                    channel_zenith = np.random.uniform(60, 85) #down
                    #channel_zenith = np.random.uniform(95, 120) #up
                    channel_azimuth = angle = np.random.uniform(0, 360)
                    ...
                    for channel_id in range(self._det.get_number_of_channels(self._station_id)):
                        ...
                        for iS in range(n):  # loop through all ray tracing solution
                            ...
                            #"""
                            #set electric field manually

                            n_samples_time = 514
                            sampling_rate_added = 2.4

                            amp_p0 = 100#
                            amp_p1 = -1#
                            phase_p1 = n_samples_time / sampling_rate_added * np.pi
                            phase_p0 = np.pi/2#
                            quadratic_term = 0#
                            quadratic_term_offset = 0#
                            zenith_arrival = channel_zenith *units.deg
                            azimuth_arrival = channel_azimuth *units.deg

                            efield_norm = get_analytic_pulse_freq(amp_p0, amp_p1, phase_p0, n_samples_time, sampling_rate_added, phase_p1=phase_p1, bandpass=None, quadratic_term=quadratic_term, quadratic_term_offset=quadratic_term_offset)
                            electric_field = NuRadioReco.framework.electric_field.ElectricField([channel_id], position=None, shower_id=self._shower_ids[self._shower_index], ray_tracing_id=iS)
                            
                            electric_field.set_frequency_spectrum(np.array([np.zeros_like(efield_norm), 
                                                                        efield_norm*channel_amplitude_theta, 
                                                                        efield_norm*channel_amplitude_phi]), 
                                                                        sampling_rate_added)
                            ...
                            direction_vector = np.array([
                                                        np.sin(zenith_arrival) * np.cos(azimuth_arrival),
                                                        np.sin(zenith_arrival) * np.sin(azimuth_arrival), 
                                                        np.cos(zenith_arrival)                     
                                                        ])

                            relative_delay = -np.dot(x2, direction_vector) / (constants.c * units.m / units.s / 1.3)
                            trace_start_time = self._vertex_time + relative_delay
...
#         self._mout['multiple_triggers'] = np.zeros((self._n_showers, self._number_of_triggers), dtype=bool)
    ...
    def _write_output_file(self, empty=False):
        ...
        if not empty:
            ...
            logger.info("start saving events")
            # save data sets
