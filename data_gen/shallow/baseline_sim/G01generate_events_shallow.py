...
phasedArrayTrigger = NuRadioReco.modules.phasedarray.triggerSimulator.triggerSimulator(log_level=logging.WARNING)
...
triggerTimeAdjuster = NuRadioReco.modules.triggerTimeAdjuster.triggerTimeAdjuster(log_level=logging.WARNING)
triggerTimeAdjuster.begin(pre_trigger_time=75*units.ns)
...
def task(q, iSim, nu_energy, nu_energy_max, detectordescription, config, output_filename,
    ...
    def get_max_radius(E):
        if(E <= 10 ** 16.21 * units.eV):
            return 0.8 * units.km
        elif(E <= 10 ** 16.41 * units.eV):
            return 1.0 * units.km
        elif(E <= 10 ** 16.61 * units.eV):
            return 1.5 * units.km
        elif(E <= 10 ** 16.81 * units.eV):
            return 1.8 * units.km
        elif(E <= 10 ** 17.01 * units.eV):
            return 2.0 * units.km
        elif(E <= 10 ** 17.21 * units.eV):
            return 2.3 * units.km
        elif(E <= 10 ** 17.41 * units.eV):
            return 2.6 * units.km
        elif(E <= 10 ** 17.61 * units.eV):
            return 2.7 * units.km
        elif(E <= 10 ** 17.81 * units.eV):
            return 2.8 * units.km
        elif(E <= 10 ** 18.01 * units.eV):
            return 2.9 * units.km
        elif(E <= 10 ** 18.21 * units.eV):
            return 3.1 * units.km
        elif(E <= 10 ** 18.41 * units.eV):
            return 3.2 * units.km
        elif(E <= 10 ** 18.61 * units.eV):
            return 3.3 * units.km
        elif(E <= 10 ** 18.81 * units.eV):
            return 3.5 * units.km
        elif(E <= 10 ** 19.01 * units.eV):
            return 3.5 * units.km
        elif(E <= 10 ** 19.21 * units.eV):
            return 3.5 * units.km
        elif(E <= 10 ** 19.41 * units.eV):
            return 3.5 * units.km
        elif(E <= 10 ** 19.61 * units.eV):
            return 4.0 * units.km
        else:
            return 4.0 * units.km
    ...
    class mySimulation(simulation.simulation):
        ...
        def _detector_simulation_trigger(self, evt, station, det):
            ...
            for channel_id in det.get_channel_ids(station_copy.get_id()):
                threshold_high[channel_id] = thresholds['2of4_100Hz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
                threshold_low[channel_id] = -thresholds['2of4_100Hz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
            ...
            threshold_low = {}
            for channel_id in det.get_channel_ids(station_copy.get_id()):
                threshold_high[channel_id] = thresholds['2of4_10mHz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
                threshold_low[channel_id] = -thresholds['2of4_10mHz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
            highLowThreshold.run(evt, station_copy, det,
                                        threshold_high=threshold_high,
                                        threshold_low=threshold_low,
                                        coinc_window=40 * units.ns,
            ...
            threshold_low = {}
            for channel_id in det.get_channel_ids(station_copy.get_id()):
                threshold_high[channel_id] = thresholds['2of4_100mHz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
                threshold_low[channel_id] = -thresholds['2of4_100mHz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
            highLowThreshold.run(evt, station_copy, det,
                                        threshold_high=threshold_high,
                                        threshold_low=threshold_low,
                                        coinc_window=40 * units.ns,
                                        triggered_channels=shallow_channels,  # select the LPDA channels
                                        number_concidences=2,  # 2/4 majority logic
                                        trigger_name='LPDA_2of4_100mHz',
                                        set_not_triggered=not station_copy.has_triggered(trigger_name='LPDA_2of4_100Hz')) # only calculate the trigger at a lower threshold if the previous one triggered

            threshold_high = {}
            threshold_low = {}
            for channel_id in det.get_channel_ids(station_copy.get_id()):
                threshold_high[channel_id] = thresholds['2of4_1Hz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
                threshold_low[channel_id] = -thresholds['2of4_1Hz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
            highLowThreshold.run(evt, station_copy, det,
                                        threshold_high=threshold_high,
                                        threshold_low=threshold_low,
                                        coinc_window=40 * units.ns,
            ...
            threshold_low = {}
            for channel_id in det.get_channel_ids(station_copy.get_id()):
                threshold_high[channel_id] = thresholds['2of4_10Hz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
                threshold_low[channel_id] = -thresholds['2of4_10Hz'] * Vrms_per_channel_copy[station_copy.get_id()][channel_id]
            highLowThreshold.run(evt, station_copy, det,
                                        threshold_high=threshold_high,
                                        threshold_low=threshold_low,
                                        coinc_window=40 * units.ns,
    ...
    r_max = get_max_radius(nu_energy_max)
    z_min = -get_max_depth(nu_energy_max)
...
if __name__ == "__main__":
    ...
    kwargs['nu_energy_max'] = args.energy_max * units.eV

    filename = os.path.splitext(os.path.basename(__file__))[0]

    output_folder = f"nu_{args.flavor}_{args.interaction_type}"
    output_path = os.path.join(output_folder, f"{np.log10(kwargs['nu_energy']):.2f}eV", f"{args.index:06}")
    ...
    # start a simulation on 20 cores with a runtime of 4days
    r = myrunner(20, task, output_path, max_runtime=3600 * 24 * 4, n_triggers_max=70000, kwargs=kwargs)
    r.run()
