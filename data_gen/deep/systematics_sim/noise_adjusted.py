...
class thermalNoiseGeneratorPhasedArray():
    ...
    def __init__(self, detector_filename, station_id, triggered_channels,
                 ...
                 n_beams=11, quantize=True, keep_full_band=False):
        """
    ...
    def __generation(self):
        ...
        for iCh in range(self.n_channels):
            ...
            trace_unfiltered = fft.freq2time(spec, self.sampling_rate * self.upsampling)
            spec *= self.filt
            ...
            if self.keep_full_band:
                self._traces_unfiltered[iCh] = trace_unfiltered
    ...
    def generate_noise(self, phasing_mode="slice", trigger_mode="binned_sum", debug=False):
        ...
        self._traces_unfiltered = np.zeros((self.n_channels, self.n_samples * self.upsampling))
        counter = 0
        ...
        while True:
            ...
            if is_triggered:
                ...
                if (i_low >= 0) and (i_high < self.n_samples): # If range is directly a subset of the waveform
                    if self.keep_full_band:
                        self._traces_unfiltered = scipy.signal.resample(self._traces_unfiltered, np.shape(self._traces_unfiltered)[-1] // self.upsampling, axis=-1)
                        return self._traces_unfiltered[:, i_low:i_high]
                    else:
                ...
                if self.keep_full_band:
                    self._traces_unfiltered = np.roll(self._traces_unfiltered, -i_low, axis=-1)
                    return self._traces_unfiltered[:, :self.n_samples_trigger]
