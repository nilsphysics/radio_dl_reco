...
parser.add_argument("systematic", type=str, default=0, help="Name a new Run to be saved")
parser.add_argument("--sample", type=int, default=0, help="Name a new Run to be saved")
parser.add_argument("--file_size", type=int, default=15000, help="Name a new Run to be saved")
...
result_dir = '~/gen2_shallow_reco/reconstruction/paper_prep/combined/results/' + args.Run_number + '/full_y_pred/systematics/'
model_file = args.systematic + '_results_' + args.epoch +'.npy' 
data_dir = '~/data/gen2_shallow_snr/syst_files'

data = np.load(os.path.join(data_dir, 'shallow_syst_' + args.systematic + '.npy' ), allow_pickle=True)
...
if args.plot_event:
    plot_energy = 17.085975646972656
    plot_zenith = 1.368759036064148
...
"""
def _add_empty_channel(station, channel_id):
    #Adds a channel with an empty trace (all zeros) to the station with the correct length and trace_start_time 
    channel = NuRadioReco.framework.channel.Channel(channel_id)
    channel.set_trace(np.zeros(samples_per_channel), sampling_rate_global)
    # we need to use any other channel to get the correct trace_start_time. All channels have the same start time at the end
    # of the simulation.
    print('station.get_channel_ids()', station.get_channel_ids())
    channel.set_trace_start_time(station.get_channel(station.get_channel_ids()[0]).get_trace_start_time())
    station.add_channel(channel)
"""
"""
def get_goodness_of_fit_new(data_trace, n_channels, n_samples, sampling_rate, shower_list, station_id, detector_file, propagator_dicr, ice_model, config_file, time_logger_numc):

    station = NuRadioReco.framework.station.Station(1001)
    sim_station = NuRadioReco.framework.sim_station.SimStation(1001)
    sim_station.set_is_neutrino()

    for channel in range(n_channels):

        raw_channel = NuRadioReco.framework.channel.Channel(channel)
        raw_channel.set_trace(data_trace[channel], sampling_rate)
        raw_channel.set_trace_start_time(0)
        station.add_channel(raw_channel)

        sim_station = sim.calculate_sim_efield(shower_list, station_id, channel,
                                detector_file, propagator_dicr, ice_model, config_file,
                                time_logger=time_logger_numc)

        if len(sim_station.get_electric_fields()) == 0:
            _add_empty_channel(sim_station, channel)
            
        else:
            sim.apply_det_response_sim(sim_station, detector_file, config_file, _detector_simulation_filter_amp)
        
        for i, ray_solution in enumerate(sim_station.iter_channels()):
            ray_solution.resample(sampling_rate)
        
        station.add_sim_station(sim_station)
            
    dll_comp, _, _, data_array, compare_signal_final = nois_class.calculate_minus_two_delta_llh_station(station, station.get_sim_station(), time_grid=None, use_channels = None, frequency_domain=False, plot=True, return_traces=True)

    return compare_signal_final, dll_comp, 1
"""
...
def _detector_simulation_filter_amp(evt, station, det):
    ...
    #channelBandPassFilter.run(evt, station, det, passband=[50 * units.MHz, 100 * units.GHz],
    #                            filter_type='butter', order=10)
...
npsd_total = np.array([npsd_lpda_low * npsd_lpda_high, npsd_lpda_low * npsd_lpda_high, npsd_lpda_low * npsd_lpda_high, npsd_lpda_low * npsd_lpda_high, npsd_vpol_low * npsd_vpol_high])# * filt
...
plt.plot(test_freq, np.abs(npsd_total[0]))
plt.savefig('spec.png')



Tnoise = 300
bandwidth = np.trapz(np.abs(npsd_total) ** 2, test_freq)
shallow_vrms = (Tnoise * 50 * constants.k * bandwidth / units.Hz) ** 0.5
print(shallow_vrms)
"""
...
noise_traces = np.load('~/data/gen2_shallow_snr/syst_files/shallow_syst_pure_noise.npy')

window = signal_processing.half_hann_window(512, half_percent=0.05)
#window = np.ones(512)
#noise_traces *= window

window_integral = np.sum(np.ones(512) - window)
...
filter_new = channelBandPassFilter.get_filter(test_freq, 0, 0, det, passband=[1 * units.MHz, 500 * units.MHz],
                                filter_type='butter', order=10)
#filter_low = channelBandPassFilter.get_filter(test_freq, 0, 0, det, passband=[50 * units.MHz, 100 * units.GHz],
#                                filter_type='butter', order=10)
...
dll_noise = nois_class.calculate_minus_two_delta_llh(noise_traces)
...
plt.axvline(matrix_rank, color='k', linestyle='--', linewidth='2', label='rank = ' + str(matrix_rank))
plt.axvline(np.median(dll_noise), color='r', linestyle='--', linewidth='2', label='median = ' + str(int(np.median(dll_noise))))
plt.plot(x, pdf1, label=f'Chi-Squared', color='tab:orange')
plt.xlabel('dll')
plt.ylabel('Events')
plt.legend()
plt.savefig('dll_noise_nowindow.png')
...
def goodness_with_showers(data, profile_ids, energy, xx, yy, zz, zenith, azimuth, with_plot=0):
    ...
    for profile_id in profile_ids:
        ...
        shower[shp.vertex] = np.array([xx*units.km, yy*units.km, zz*units.km])
        shower[shp.type] = 'had'
        ...
        #_, goodness_of_fit_new, _ = get_goodness_of_fit_new(data, number_of_channels, samples_per_channel, sampling_rate_global, showers, sid, det, propagator, ice, cfg, time_logger)
        #print('goodness_of_fit', goodness_of_fit)
        #print('goodness_of_fit_new', goodness_of_fit_new)
        ...
        try:
            ...
            compare_signal_dict[int(profile_id)] = np.zeros((5, 512))
    ...
    if with_plot:
        return goodness_of_fit_final, compare_signal_final
...
def task_function(indx):
    ...
    if true_flavor == 1:
        ...
        #print(true_zenith)
        #print(np.round(true_zenith, 1))
        #print(plot_zenith)
        #print(np.round(plot_zenith, 1))
        ...
        if plot_zenith == true_zenith:
            ...
            print(true_zenith)
            print(true_energy)
            goodness_mode, signal_trace = goodness_with_showers(event_data, profile_numbers, mode_energy, mode_xx, mode_yy, mode_zz, mode_zenith, mode_azimuth, with_plot=1)

            save_dir = {}
            save_dir['shallow'] = {}
            save_dir['shallow']['signal'] = signal_trace
            save_dir['shallow']['data'] = event_data
            save_dir['shallow']['goodness_mode'] = goodness_mode
            np.save('~/gen2_shallow_reco/reconstruction/paper_prep/paper_plots/example_event_data/shallow_example.npy', save_dir)


            fig, axes = plt.subplots(5, 1, sharex=True, figsize=(5, 5))  # 5 rows, 1 column
        ...
        true_energy = result_dict['true_energy'][indx]
        true_xx = result_dict['true_xx'][indx]#/1000
        true_yy = result_dict['true_yy'][indx]#/1000
        true_zz = result_dict['true_zz'][indx]#/1000
        true_azimuth = result_dict['true_azimuth'][indx]
        true_zenith = result_dict['true_zenith'][indx]
...
def main_multiprocessing():
    ...
    mode_array = np.zeros([args.file_size])  # Initialize the shared numpy array

    # Specify the number of CPUs (e.g., use 4 CPUs)
    num_cpus = 30  # You can change this number or use os.cpu_count() for all available CPUs
    ...
    if args.plot_event ==0:
        np.save(result_dir + 'syst_goodness_' + args.systematic + '.npy' , goodness_dict)
