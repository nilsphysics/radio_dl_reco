...
save_path = '~/gen2_shallow_reco/reconstruction/paper_prep/combined/results/' + args.Run_number + '/full_y_pred/systematics/'
...
with torch.no_grad():
    for count, test_quant in enumerate(tqdm(test_loader)):
        ...
        batch_length = args.test_batchSize
        ...
        results_dict['channel_0_max_manual'].extend(result_snr[0])
        results_dict['channel_1_max_manual'].extend(result_snr[1])
        results_dict['channel_2_max_manual'].extend(result_snr[2])
        results_dict['channel_3_max_manual'].extend(result_snr[3])
        results_dict['channel_4_max_manual'].extend(result_snr[4])
        ...
        for indx, event in enumerate(coverage_dict_direction['pdf_scan_positions']):
            ...
            """
            if pred_energy_max[0] - true_energy[indx].item() > 0.3:
                print(pred_energy_max[0])
                print(true_energy[indx].item())
                print(true_xx[indx].item())
                print(true_yy[indx].item())
                print(true_zz[indx].item())
            else:
                continue
            quit()
            """
            ...
            """
            if (np.round(true_xx[indx].item(), 4) == np.round(-0.19635522365570068, 4)) & (np.round(true_yy[indx].item(), 4) == np.round(-0.38709717988967896, 4)) & (np.round(true_zz[indx].item(), 4) == np.round(-0.8876270651817322, 4)):
                print(pred_energy_max[0])
                print(true_energy[indx].item())
                print(true_xx[indx].item())
                print(true_yy[indx].item())
