...
train = data_gen.Prepare_Dataset_iterable(file_ids=list_of_file_ids_train, data_type = '_training')
val = data_gen.Prepare_Dataset_iterable(file_ids=list_of_file_ids_val, data_type = '_validating')
...
mc = ModelCheckpoint(dirpath=saved_model_dir, filename= '{epoch}_model_checkpoint', 
    monitor='val_weighted_loss', verbose=1, every_n_epochs = 1, save_top_k = 3)#, save_top_k=3)

es = EarlyStopping("val_weighted_loss", patience=hyperparameters.es_patience, min_delta=hyperparameters.es_min_delta, verbose=1)
