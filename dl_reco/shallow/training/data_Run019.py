...
class Prepare_Dataset_iterable(IterableDataset):
    def __init__(self, file_ids, data_type = '_training'):

        self.file_ids = file_ids
        self.data_type = data_type

    def filter_fn(self, data, times):
        ...
        all_masks['nan_mask'] = ~nan_mask.any(dim=1).any(dim=1).any(dim=1)
    ...
    def load_file(self, i_file, norm=1e-6):
        ...
        snr = np.array([channel_0_max, channel_1_max, channel_2_max, channel_3_max, channel_4_max])
    ...
    def __iter__(self):
        ...
        if worker_info is None:
            assigned_file_ids = self.file_ids
        else:
            num_workers = worker_info.num_workers
            worker_id = worker_info.id
            assigned_file_ids = self.file_ids[worker_id::num_workers]

        for file_index in self.file_ids:
            # Dynamically load the file
            data, direction, flavor, energy_and_vertex, snr, times = self.load_file(file_index, hyperparameters.norm)

            # Apply filtering
            filter_mask = self.filter_fn(data, times)
            ...
            # Shuffle events within the file
            indices = torch.randperm(filtered_data.shape[0])  # Shuffle indices
            for i in indices:
                yield (
...
class Prepare_Dataset_iterable_test(IterableDataset):
    def __init__(self, file_ids, data_type = '_training', shuffle=True):
    ...
    def filter_fn(self, data, times):
        ...
        if (hyperparameters.filter_times == 1) & (self.data_type == '_training'):
            times_tensor = torch.tensor(times, dtype=torch.float32)
            diff_time = times_tensor[1] - times_tensor[0]
            all_masks['time_mask'] = (diff_time > -120) & (diff_time < 20)
    ...
    def __iter__(self):
        ...
        for file_index in self.file_ids:
            ...
            if self.shuffle:
                ...
                indices = torch.arange(filtered_data.shape[0])
...
class Prepare_Dataset_direction(Dataset):
    ...
    def load_file(self, i_file, norm=1e-6):
        ...
        #print(f"{hyperparameters.data_name}{hyperparameters.interaction}{self.data_type}{'_10k_'}{i_file}_labels.npy")
        ...
        #max_norm = np.max(data, axis=(1, 2, 3))
        #max_norm=max_norm[:,None,None,None]
...
class Prepare_Dataset_systematics(IterableDataset):
    ...
    def load_file(self, norm=1e-6):
        ...
        data_location = '~/data/gen2_shallow_snr/syst_files'

        data = np.load(os.path.join(data_location, 'shallow_syst_' + self.syst_name + '.npy' ), allow_pickle=True)
        labels_tmp = np.load(os.path.join(data_location, 'shallow_syst_' + self.syst_name + '_labels.npy' ), allow_pickle=True)
    ...
    def __iter__(self):
        ...
        #only use for plane wave
        for i in range(data.shape[0]):  # Iterate sequentially
            yield (
                torch.from_numpy(data[i]).to(torch.float32),
                torch.from_numpy(direction[:, i]).to(torch.float32),
                torch.tensor(flavor[i], dtype=torch.float32),  # Handle the scalar as a tensor
                torch.from_numpy(energy_and_vertex[:, i]).to(torch.float32),
                torch.from_numpy(snr[:, i]).to(torch.float32),
                torch.from_numpy(np.ones((2, 1024))),
            )
